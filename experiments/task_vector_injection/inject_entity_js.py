"""
Distribution-level behavioral test for entity→attribute injection.
...
distribution under injection to its distribution under actual ICL:
  ...
  Z          first-token dist for the bare country prompt (zero-shot)
  P_ICL_g    first-token dist for a pure task-g ICL prompt + the country
  Q_inject_t first-token dist for the bare country prompt with contrast vector t
             injected (additively) at the assistant-header token of `layer`

Confusion matrix (per country):
...
Significance: bootstrap over countries (default 1000 resamples) for a 95% CI on
each double-centered cell; the diagonal is significant if its CI upper bound < 0.

Pure ICL tasks only, for now.

Usage:
  python experiments/task_vector_injection/inject_entity_js.py --layer 18 --alphas 4
  python experiments/task_vector_injection/inject_entity_js.py --layer 18 --alphas 1 2 4 6
"""
...
_p.add_argument("--model", default=os.environ["TASK_VECTOR_MODEL"],
                help="HuggingFace model ID. Must match the model used to run "
                     "extract_entity.py, since vectors are looked up by MODEL_TAG.")
_p.add_argument("--layers", type=int, nargs="+", default=None,
                help="Layers to inject at (sweep). Default: the aligned relative-depth "
                     "set, same as sweep_entity.py / extract_entity.py.")
_p.add_argument("--alphas", type=float, nargs="+", default=[4.0])
_p.add_argument("--n_icl", type=int, default=4,
                help="ICL demo samplings to average per (country, task) for P_ICL_g.")
_p.add_argument("--n_countries", type=int, default=None)
_p.add_argument("--n_bootstrap", type=int, default=1000)
_p.add_argument("--n_perm", type=int, default=10000,
...
_p.add_argument("--all_ratios", action="store_true",
...
_p.add_argument("--ceiling", action="store_true",
...
import numpy as np
import torch
from local_model import model, tokenizer, _device, n_layers
...
def load_vec(task: str, layer: int) -> torch.Tensor:
    fname = (f"entity_contrast_{task}_layer{layer}.npy" if _IS_DEFAULT
             else f"entity_contrast_{task}_{MODEL_TAG}_layer{layer}.npy")
    path = os.path.join(VEC_DIR, fname)
    if not os.path.exists(path):
        raise FileNotFoundError(f"No stored vector at {path}. Run extract_entity.py --model {_args.model} first.")
...
def build_mixed_icl(pools, rng, ratio, test_rec):
    """ICL prompt with demos in the given task ratio + the test country as the
    final user turn. Handles pure and mixed ratios (pure = 3 demos of one task)."""
    demos = []
    for task, n in zip(TASKS, ratio):
        pool = [c for c in pools[task]["icl"] if c["country"] != test_rec["country"]]
    ...
    # Test input is a bare country name (task-independent).
    msgs.append({"role": "user", "content": data.format_test_input(TASKS[0], test_rec)})
    return msgs
...
def double_center(mat: np.ndarray) -> np.ndarray:
    ...
    return (mat - mat.mean(axis=1, keepdims=True)
                - mat.mean(axis=0, keepdims=True) + mat.mean())
...
def ceiling_diagnostic(pools, countries, ratio_set, n_icl):
    """How distinguishable are the real P_ICL(r) distributions from each other?

    Computes the mean (over countries) JS matrix among the P_ICL(r) — no injection,
    independent of layer/alpha. Returns (mean_js, simplex_dist, corr) where corr is
    the correlation between off-diagonal JS and simplex distance: a strong positive
    corr means behavioral distinguishability tracks task composition; uniformly tiny
    JS means a low behavioral ceiling (injection can't reproduce what ICL barely
    differentiates)."""
    R = len(ratio_set)
    mats = []
    for rec in countries:
        ...
        for ri, r in enumerate(ratio_set):
            dists = [first_token_dist(build_mixed_icl(pools, random.Random(1000 * ri + s), r, rec))
    ...
    dist    = np.array([[simplex_distance(ratio_set[i], ratio_set[j]) for j in range(R)]
                        for i in range(R)])
...
def per_country_matrices(pools, countries, vecs, ratio_set, layer, alpha, n_icl):
    """Return list of R×R raw JS matrices M[r][r'] = JS(Q_inject(r), P_ICL(r')),
    one per country, where R = len(ratio_set).

    Q_inject(r) injects the weighted-sum vector Σ w_i v_i; P_ICL(r') is the real
    ratio-r' ICL output distribution, averaged over n_icl demo samplings."""
    mats = []
    for rec in countries:
        ...
        for ri, r in enumerate(ratio_set):
            ...
            for s in range(n_icl):
                ...
                dists.append(first_token_dist(build_mixed_icl(pools, rng, r, rec)))
            P[r] = np.mean(dists, axis=0)
        # Q_inject(r): inject the weighted-sum vector for ratio r
        Q = {r: first_token_dist(prompt, inject_vector(r, vecs), layer, alpha)
             for r in ratio_set}
        M = np.array([[js_divergence(Q[r], P[rp]) for rp in ratio_set] for r in ratio_set])
        mats.append(M)
    return mats
...
def bootstrap_double_centered(mats, n_bootstrap):
    ...
    resampling countries with replacement."""
    arr   = np.stack(mats)                 # (n_countries, R, R)
    R     = arr.shape[1]
...
def permutation_test(mats, n_perm=10000, seed=0):
    ...
    the LOWEST (closest in JS). Per country, the column labels are exchangeable
    under the null (injection is ratio-agnostic), so each null draw assigns every
    country an independent uniform column permutation and sums the traces.
    ...
    # Independent random column permutation per (draw, country) via argsort trick.
...
def print_results(point, lo, hi, raw_mean, labels):
    ...
    for ri, r in enumerate(labels):
        ...
        cells = "  ".join(f"{raw_mean[ri, ci]:>7.4f}{'*' if ci == amin else ' '}"
...
if __name__ == "__main__":
    ...
    layers = _args.layers or sorted(set(max(1, round(f * n_layers)) for f in _LAYER_FRACS))

    ratio_set = list(data.RATIOS) if _args.all_ratios else list(data.PURE_RATIOS)
    labels    = [ratio_label(r) for r in ratio_set]
    R         = len(ratio_set)

    print(f"JS injection test | {'all 10 ratios' if _args.all_ratios else '3 pure tasks'} | "
          f"layers {layers} | alphas {_args.alphas} | n_icl={_args.n_icl} | "
          f"n_bootstrap={_args.n_bootstrap} | n_perm={_args.n_perm}")

    data_pools = data.load_pools()
    countries  = data_pools[TASKS[0]]["test"]
    if _args.n_countries:
        countries = countries[:_args.n_countries]
    print(f"Test countries: {len(countries)}\n")

    if _args.ceiling:
        print("Ceiling diagnostic: JS among real P_ICL(r) distributions (no injection)\n")
        mean_js, dist, corr = ceiling_diagnostic(data_pools, countries, ratio_set, _args.n_icl)
        ...
        iu    = np.triu_indices(R, k=1)
        pairs = sorted(((mean_js[i, j], labels[i], labels[j], dist[i, j])
                        for i, j in zip(*iu)), key=lambda x: x[0])
    ...
    for layer in layers:
        try:
            ...
            print(f"layer {layer:>2}: SKIP (no stored vector — re-run extract_entity.py)")
            continue
        for alpha in _args.alphas:
            mats = per_country_matrices(data_pools, countries, vecs, ratio_set, layer, alpha, _args.n_icl)
            point, lo, hi, raw_mean, boot = bootstrap_double_centered(mats, _args.n_bootstrap)
            ...
            print(f"layer {layer:>2}, alpha {alpha:>4}: mean centered diag = "
    ...
    print("  bootstrap meanDiag (effect size, more negative = stronger) | permutation p (significance)")
    print(f"{'layer':>6} {'alpha':>6} {'meanDiag':>10} {'95% CI':>22} {'row-min':>8} {'perm-p':>8}")
    for (layer, alpha, mdiag, mlo, mhi, rowmin, sig, perm_p) in curve:
        star = "*" if perm_p < 0.05 else " "
        print(f"{layer:>6} {alpha:>6} {mdiag:>+10.4f}  [{mlo:>+7.4f}, {mhi:>+7.4f}]  "
    ...
    if _args.all_ratios:
        print("Note: with 10 ratios, adjacent ratios (e.g. 2-1-0 vs 1-2-0) are genuinely "
