"""
Layer-by-layer scatter of contrast vector projections for arithmetic format
tasks (Direct / MCQ / Verification) across all ICL ratios.

For each layer in SWEEP_LAYERS:
  - Fits LDA on the three pure-condition contrast vectors
  - Projects all 10 ratios (pure + mixed) into that 2D space
  - Plots individual samples, pure-task centroids, and actual vs predicted
    mixed centroids (connected by dotted lines)

All layers are captured in a single pair of forward passes per sample,
so total cost is 2 x N_SAMPLES x N_RATIOS forward passes regardless of
how many layers are swept.

Layers are chosen as fixed fractions of model depth so results are
...
Usage:
    ...
    python experiments/sweep_arithmetic.py --model meta-llama/Llama-3.1-8B-Instruct
"""
...
_PURE_RGB = np.array([
    [0.27, 0.51, 0.71],   # steelblue  → Direct
    [0.18, 0.55, 0.34],   # seagreen   → MCQ
    [0.85, 0.55, 0.13],   # darkorange → Verification
])
...
def collect_contrasts(
    ...
    for a given sample index. Reduces total passes from 2*n_ratios*n by
    roughly half.
    """
    # --- Precompute zero-shot activations (n_samples passes, shared across ratios) ---
    ...
    for i in range(n_samples):
        ...
        # test_task is fixed by sample_idx (rotate_test=True) and test_ex is
        # the first rng call — ratio-independent.
        rng       = random.Random(i)
        test_task = TASKS[i % len(TASKS)]
        test_ex   = rng.choice(pools[test_task]["test"])
        zs_msgs   = [{"role": "user", "content": format_test_input(test_task, test_ex)}]
...
def plot_layer_grid(results: dict, layers: list[int]) -> None:
    ...
    for ax_idx, layer in enumerate(layers):
        ...
        for ratio in RATIOS:
            ...
            # Undefined (shown as "—") when predicted ≈ triangle center (e.g. 1-1-1).
    ...
    fig.suptitle(
        "Arithmetic contrast vectors — LDA projections by layer\n"
    ...
    out = os.path.join(_results_dir, f"arithmetic_layer_sweep_projections_{MODEL_TAG}_rotated.png")
    plt.savefig(out, dpi=150)
    plt.close()
    print(f"\nSaved → {out}")

    # Save κ data for cross-model comparison plot
    json_out = os.path.join(_results_dir, f"arithmetic_kappa_{MODEL_TAG}.json")
...
if __name__ == "__main__":
    print("Loading arithmetic pools...")
    pools = load_pools()
